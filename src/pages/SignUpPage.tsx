...
const schema = yup.object({
    fullName: yup.string().required('This email already registered'),
    ...
    return (
        <>
            <LayoutAuthentication header="Sign Up">
                <p className="text-center text-text3 text-xs lg:text-sm mb-[25px] lg:mb-[30px]">Already have an account? <Link className="text-primary" to="/sign-in">Sign in</Link></p>
                <div className="cursor-pointer flex items-center justify-center gap-[10px] border dark:border-darkStroke border-stock rounded-[10px] mb-[10px] ">
                    ...
                    <span className="py-[13px] text-center text-base font-bold text-text2 dark:text-white">Sign up with google</span>
                </div>
                <div className="text-text2 lg:text-sm text-xs text-center p-[10px] dark:text-white">Or sign up with email</div>
                <form onSubmit={handleSubmit(onSubmit)}>
                    <FormGroup>
                        <Label htmlFor="fullName">Full Name *</Label>
                        <Input control={control} type="text" errors={errors} id="fullName" name="fullName" placeholder="John Doe" />
                    </FormGroup>
                    <FormGroup>
                    ...
                    <Button type="submit" className="w-full mt-[25px] lg:mt-[20px]" loading={isLoading}>Create my account</Button>
                </form>
